...
/// Origin pengganti selama perangkat belum terhubung ke database. Ia hanya
/// menjadi kunci identitas lokal (`desktop_client_identity`, vault offline) dan
/// tidak pernah dihubungi: TLD `.invalid` dicadangkan RFC 2606 dan tidak bisa
/// di-resolve. Dulu nilainya URL server web Vercel, sisa jalur server aplikasi
/// yang sudah dipensiunkan.
const UNCONFIGURED_ORIGIN: &str = "http://database-belum-diatur.invalid";

/// Galat tunggal untuk perangkat yang belum terhubung ke database mana pun.
/// Kodenya tetap `TURSO_NOT_CONFIGURED` karena frontend dan `bootstrap_status`
/// sudah mengenalinya.
pub fn database_not_configured() -> CommandError {
    CommandError::new(
        "TURSO_NOT_CONFIGURED",
        "Perangkat belum terhubung ke database. Atur koneksi (Turso Cloud, Server Database Sendiri, atau Database Lokal) terlebih dahulu.",
    )
}
...
impl DesktopState {
    pub fn initialize(app: &AppHandle) -> Result<Self, String> {
        ...
        let temp_state = Self {
            server_origin: std::sync::RwLock::new(UNCONFIGURED_ORIGIN.into()),
...
mod tests {
    ...
    use super::{
        parse_bool_setting, parse_offline_hours_value, parse_provider_setting,
        DatabaseProvider,
    };
